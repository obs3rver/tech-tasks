package pl.techgarden.tasks.geolocation.metrics;
...
class MetricsLoggingAspect {
    ...
    @Pointcut("@within(pl.techgarden.tasks.geolocation.metrics.EnableMetricsLog)")
    void logAnnotation() {
    }
    ...
    private void log(ProceedingJoinPoint joinPoint, StopWatch stopWatch, Object result) {
        ...
        if (!timerString.isEmpty() || !resultString.isEmpty()) {
            ...
            logger.debug("{}{} {}", operationName, timerString, resultString);
        }
    }
    ...
    private String getOperationName(ProceedingJoinPoint joinPoint) {
        String classWithPackageName = joinPoint
                .getSignature()
                .getDeclaringTypeName();
        String className = classWithPackageName
                .substring(classWithPackageName.lastIndexOf('.') + 1);
